...
/**
 * 业务错误码：插件自管，须 >= PLUGIN_ERROR_START(-3001)，宿主不解释其含义。
 * 通过 plugin-sdk 的 PluginError（code + msg）携带；msg 可为字符串或多语言对象。
 */
const PluginErrCode = {
  NOT_FOUND: -3001,
...
rpc.registerHandler('__PLUGIN_ID__.greet', () => {
  ...
  // 成功：封装为 { code: 0, data, from }（也可直接 return 裸数据，SDK 会自动包成信封）
  return rpc.success(`Hello from __PLUGIN_ID__ worker`)
})

rpc.registerHandler('__PLUGIN_ID__.echo', () => {
  void rpc.log.write('info', 'echo called', { from: 'worker' })
  // 主动失败：rpc.error(code, msg) —— 业务码 + 多语言 msg，渲染层按 locale 解析
...
rpc.registerHandler('__PLUGIN_ID__.guard', () => {
  ...
  // 业务错误：插件侧抛 PluginError，SDK 兜底包装为失败信封（code ≥ -3001），不会向外 reject
