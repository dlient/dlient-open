...
description: Develop plugins for the dlient desktop host. Covers scaffolding with @dlient-open/create-plugin, the plugin manifest schema, worker (Node) and UI (SystemJS remoteEntry) development, host-api calls, and the sandbox + permission model. Use when creating, building, debugging, or distributing a dlient plugin.
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
- a **worker**: runs as a sandboxed Node process (no DOM, no renderer);
...
1. **Sandbox first**: inside the worker you cannot use `node:fs` / `node:child_process`, cannot load `.node` addons, and cannot block synchronously — everything goes through host-api.
...
- **Distributing a plugin**: publish it to **npm** — the host console has a built-in **NPM market** tab that searches `keywords:dlient-open-plugin` packages and supports install / update — or build it as a `.dlient` package to share or import locally.
...
| Worker pool      | `workerMode: 'solo'` gives a plugin an exclusive process (one plugin, one process); default `shared` groups low-sensitivity plugins together. `source: 'dev'` plugins are forced solo.           |
...
| Permission model | Process-level bans (no `node:fs` / `node:child_process`, no `.node` addons, no sync blocking) + resource-level grants (`fs-grants` / `net-grants` / `spawn-grants` / session / temp). See `references/permission-model.md`. |
...
**Expose policy** — add `dlient.expose` entries only when another plugin genuinely must call this one, or the user explicitly asks. A plugin can expose methods **if and only if it actually ships a worker** — i.e. its build output contains `dist/worker.js`. The manifest `type` is irrelevant: an `app`- or `ui`-typed plugin that ships `dist/worker.js` **can** expose (the worker registers the handlers), while a plugin with no `dist/worker.js` cannot — nothing can serve the call, and the host rejects it with `WORKER_NOT_RUNNING` (`-2102`). Prefer to also expose the `grant` handler (recommended, not required) and prefer `ask`; return `deny` for anything touching secrets. Secrets themselves must be stored encrypted via `app.crypt`, never plaintext in `app.data` or in logs.
...
From the renderer (UI) — only the low-risk subset on the UI-open list can be called directly (`references/ui-api.md`); anything outside that list must go through the worker:

```tsx
import { useDlientApi } from '@dlient-open/api-bridge' // also re-exported by '@dlient-open/ui'

// inside a React component
const api = useDlientApi()

const text = await api.fs.read('C:/tmp/a.txt')        // needs fs.read
await api.fs.write('C:/tmp/out.bin', { base64: '…' }) // needs fs.write
const p = await api.app.getPath('userData')           // needs app.getPath
```
...
const p = await rpc.app.getPath('userData')           // needs app.getPath
```
...
| `references/native-host.md`        | Running native modules: native-host architecture, server & client, restartable host, security (expose policy & `grant`, encrypted secrets) |
| `references/worker.md`             | Writing the worker: SDK API, RPC, logging, subprocess handles, cross-plugin calls, expose policy & `grant`, secret storage |
| `references/mode-switch-worker.md` | Converting a UI-only project to the worker mode                                               |
| `references/mode-switch-native-host.md` | Converting a UI-only project to the native-host mode                                     |
| `references/mode-switch-native.md` | Converting a project to the vendored-native mode                                          |
...
| `references/npm-market.md`         | NPM market: search rules, category keywords, and the `keywords` requirements for publishing a plugin |
| `example/`                         | Copy-ready worker and native-host example sources                                             |
