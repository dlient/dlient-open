...
Inside a worker you cannot use `node:fs` / `node:child_process` / `worker_threads`, cannot load `.node` addons, and cannot block synchronously — everything goes through host-api.

- File writes, subprocesses and native modules must go through the host (`fs.*`, `child.*`) or the **native-host** child (full-permission official Node process for `.node`).
- Direct `node:net` / `fetch` is currently allowed (temporary policy; many npm packages connect directly). Host `net.*` grants remain the primary path and will replace it in strict mode.
- Dev-only escape hatches: `DLIENT_DISABLE_PERMISSION=1` (skip the process-level bans), `DLIENT_DISABLE_FS_ENFORCE=1` (skip fs path checks).
...
The host manages it (owner-tracked) and **auto-restarts it on crash**; in-flight RPC requests are rejected on restart.

## 2. Two-layer authorization

1. **Method permissions** — `manifest.permissions` entries, checked for every host-api call.
...
## 8. Secret storage — `app.crypt`

Passwords, keys and tokens must never be stored or handled in the clear:

- `app.crypt.encrypt(plain)` / `app.crypt.decrypt(ciphertext)` — encrypt/decrypt values for this plugin; permission `app.crypt`. Reachable from the worker (`rpc.app.crypt.encrypt(...)`) and from the UI (`api.crypt.encrypt(...)`, it is in the UI host-api whitelist).
- `app.data.read` / `app.data.write` is **plaintext isolated storage on disk**, so it is not enough on its own — encrypt before writing a secret, decrypt after reading it.
- There is **no permission-free alternative**: never hardcode keys, passwords or tokens in source.
- Secrets must **never** appear in logs or in cross-plugin responses.
