...
The **Permission** column in the tables below is the declaration that covers the method (a group prefix such as `app.data` also covers `app.data.*`). **`—` does NOT mean "permission-free"** — it means no group prefix covers that method, so the method key itself must be declared exactly (e.g. `app.getPath`, `app.notify`, `app.getVersion`). Nothing bypasses check 1; the only exemptions are the base capabilities `plugin.invoke` / `plugin.requestGrant` and `notification.subscribe` / `notification.unsubscribe` (owner-checked).
...
const userData = await rpc.app.getPath('userData')          // needs `app.getPath` declared
```
...
Workers currently allow direct `node:net` / `fetch` (temporary policy; many npm packages connect directly). The host `net.*` stays available for gated access.
...
| `app.crypt.encrypt` / `app.crypt.decrypt` | `(payload)` | `app.crypt` | Encrypt/decrypt values for this plugin |
...
`app.crypt.encrypt(plain)` / `app.crypt.decrypt(ciphertext)` — encrypt/decrypt values for this plugin (permission `app.crypt`; also callable from the UI as `api.crypt.*`). Use it for passwords / keys / tokens. The same plugin can decrypt its own ciphertext across sessions/restarts; the ciphertext is **per-plugin**, so another plugin cannot decrypt it.
...
- A native-host child is a hosted spawn too: decrypt in the worker, inject via `rpc.child.spawn` env. The child itself cannot call `app.crypt` — it only receives the decrypted value for the process lifetime.
- Plugin isolation: ciphertext produced by plugin A cannot be decrypted by plugin B. Do not hand ciphertext to another plugin expecting it to decrypt.
