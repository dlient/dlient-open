...
**Expose policy** — expose as little as possible: only add `expose` entries when another plugin genuinely must call this one, or the user explicitly asks. A plugin can expose methods **if and only if it actually ships a worker** — i.e. its build output contains `dist/worker.js`; the manifest `type` is irrelevant (an `app`- or `ui`-typed plugin that ships `dist/worker.js` **can** expose, because the worker registers the handlers, while a plugin with no `dist/worker.js` cannot — nothing can serve the call → the host rejects it with `WORKER_NOT_RUNNING`, `-2102`). Worker-capable plugins (`full` / `worker`, or any plugin that ships `dist/worker.js`) are the ones that normally declare `expose`, and every exposed method needs a matching `rpc.registerHandler('<pluginId>.<method>', …)` in the worker.

**Reserved `grant` key** — `expose` may contain an entry literally named `grant`, e.g. `"grant": { "description": "…", "access": "default" }`. The host invokes the target worker's `grant` handler for authorization; it returns `allow` (permanent grant for that caller + method) / `ask` (user three-way confirmation: deny / allow once / always allow) / `deny` (refused). Recommended default: **`ask`**; return **`deny`** for anything touching privacy, passwords, keys or tokens; `allow` only for clearly harmless methods. Without an exposed `grant`, an unauthorized cross-plugin call fails with `ACCESS_DENIED`.

**Secrets** — store passwords / keys / tokens encrypted via `app.crypt` (permission `app.crypt`); never write plaintext secrets into `app.data`.
...
| App | `app.data` `app.crypt` `app.window` `app.menu` `app.shortcut.register` `app.setAutoLaunch` `app.event` | group perms cover `app.data.*` / `app.crypt.*` / `app.window.*` etc. `app.getPath`, `app.notify` have no group prefix, so they must be declared as exact keys as well (no resource grant needed, but the declaration is required) |
...
| Notification | `notification.send` `notification.remove` `notification.removeGroup` | no `—`: each needs its exact key declared (or the `notification` group prefix). `notification.subscribe` / `notification.unsubscribe` are the only exempt ones (owner-checked, called internally by the handle's `on()`) |
