...
> **Don't write `dui:*` utility classes in your own markup.** The shared stylesheet is precompiled from this package's own sources, so only the utilities it already uses exist — any other `dui:*` class silently resolves to nothing. Style your own layout with plain CSS / `*.module.css` (see `dev-standards.md` → Styles & theming) and take the visuals from the components.
...
- Icons — **use lucide** instead of drawing your own SVGs. **The whole lucide set is re-exported**, so any icon is available without adding a dependency of your own:
  ```tsx
  ...
  import { WandSparkles, Trash2Icon } from '@dlient-open/ui'           // any lucide icon
  ```
  - Three aliases per icon: `WandSparkles` / `WandSparklesIcon` / `LucideWandSparkles`. For the few names ui components also use (`Table` `Sheet` `Sidebar` `Command` `Calendar` `Badge`) take the icon via `LucideXxx` (e.g. `LucideTable`).
  - `<Icon name="…">` accepts any lucide name — kebab-case, PascalCase or snake_case (`wand-sparkles` / `WandSparkles` / `wand_sparkles`); unknown names render nothing and log a warning.
