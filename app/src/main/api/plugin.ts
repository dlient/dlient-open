...
export const pluginApis: ApiDefinition[] = [
  ...
  { ...dev('plugin.install', '安装插件（.dlient/npm/github/URL，含 preInstall 依赖）', 'Install plugin (.dlient/npm/github/URL, incl. preInstall deps)'), handler: ([req]) => installPluginViaHost(req as { id: string; kind: 'file' | 'npm' | 'github' | 'url'; source: string; description: string }) },
