...
- All other whitelist-excluded host-apis (`child.*`, `app.createNative*`, `app.window.*`, `screen.*`, `webview.*`, `plugin.invoke`, …): call through the worker via `rpc.{module}.{method}` (the `UI_OPEN_METHODS` list in `app/src/main/export.ts` is the single source of truth).
