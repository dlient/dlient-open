...
Embed web content by rendering the **`Webview` component from `@dlient-open/ui`** in your UI (never create views manually — see `references/ui.md` §2.1). Once a view is inserted, control its visibility in two ways:

- **Host-level show/hide (multi-view / orchestration, e.g. tab bars)** — call these two methods (permission `webview.create`):

| Method | Args (types) | Permission | Purpose |
| --- | --- | --- | --- |
| `webview.showWebviewByPlugin` | `(views?: string[])` | `webview.create` | Show this plugin's views (restore exactly the listed cached `viewId`s when provided) |
| `webview.hideWebviewByPlugin` | `()` | `webview.create` | Hide this plugin's views → returns the hidden `viewId` list for later exact restore |

- **Component-level** — pass the `visible` prop to the `Webview` component (`<Webview src={url} visible={activeTab === id} />`); the component syncs visibility to the main process for you.

Keep a view mounted and toggle `visible` to preserve its page state; unmount only to destroy it. Remember the `viewId` from `Webview`'s `onViewReady` when you need host-level exact restoration.
