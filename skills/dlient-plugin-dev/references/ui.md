...
Puts a remote page into a main-process `WebContentsView` layered over the host window (a real browser view, not an iframe). Requires `manifest.permissions` `webview.create` (`webContents` calls additionally need `webview.navigate`).
...
- **Component-level**: for a persistently mounted `Webview`, pass `visible={activeTab === id}` and the component syncs to the main process (`webview.setVisible`). A 0×0 (hidden) container skips bounds updates to avoid reloading the page on restore.
- **Host-level exact restore (multi-view / tab orchestration)**: record `viewId` from `onViewReady`; on hide call host hide-for-plugin, on switch back pass the cached view id for **exact restore** (`webview.showWebviewByPlugin` / `webview.hideWebviewByPlugin`, permission `webview.create`).
